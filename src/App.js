...
import {isClickedMine, endGame} from './inGame';
...
function App() {
  ...
  const assignBombsAndNumbers = (index) => {
    ...
    *Of course, they will be hidden from the client in the beginnig */
    ...
    setMatrixSquare(matrixSquare)
  }
  const clikedSquare =async (index) => {
  ...
  const handleGame = (index) => {
    ...
    console.log("Pressed index: " + index)
      ...
      if(response) {
        setMatrixSquare(endGame(matrixSquare, matrixColumns))
        setGameOver(true);
      }
  }
  return (
    <div className="App background title ">
      ...
      {gameOver && <div className="game-over"><h3>Game Over</h3></div>}
